...
inline istream& operator>>(istringstream & iss, Date & date) {
  char delim{};
  return iss >> date.year >> delim >> date.month >> delim >> date.day;
}
inline istream& operator>>(istringstream & iss, Time & time) {
  iss >> time.hours;
  iss.ignore(1);
  iss >> time.minutes;
  return time;
...
void TestUpdate() {
  ...
  ASSERT_EQUAL(t.departure_time, (Time{12, 40}));
