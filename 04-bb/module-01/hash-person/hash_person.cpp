...
struct AddressHasher {
    size_t operator()(const Address &address) const {
        ...
        const auto coeff = 514'229;
        return coeff * coeff * string_hasher(address.city) +
               coeff * string_hasher(address.street) +
...
struct PersonHasher {
    size_t operator()(const Person &person) const {
        ...
        return coeff * coeff * coeff * string_hasher(person.name) +
               coeff * coeff * int_hasher(person.height) +
               coeff * double_hasher(person.weight) +
