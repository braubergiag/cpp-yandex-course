    ...
    while (first != last) {
        if (pred(*first)) {
            if (!is_found) {
                max_element = first;
                is_found = true;
            } else if (*first > *max_element)
                max_element = first;
        }
        ++first;
    }
    if (is_found) return max_element;
    else return last;
}
#if 0

void TestSimple(){
    auto alwaysTrue = [](int x) {return x % 2 == 0;};
    std::vector<int> v{19,12,10};
    auto max_iterator = v.begin();
    advance(max_iterator,1);
    Assert(max_element_if(v.begin(),v.end(),alwaysTrue) == max_iterator, "TestSimple");
}
